...
COMPARISON_METRIC = 'luminance_ssim'
...
def save_to_csv(mean_list, std_list, accuracy_list, augmentation_type, augmentation_magnitudes_list, time_list):
    ...
    with open(filename, mode="w", newline="") as file:
        ...
        for i, (mean, std, accuracy, augmentation_magnitude, time_taken) in enumerate(
            zip(mean_list, std_list, accuracy_list, augmentation_magnitudes_list, time_list), 1
        ):
            writer.writerow([augmentation_magnitude.item(), mean, std, accuracy, time_taken])
    print(f"Results saved to {filename}")
    return filename


def plot_mean_std(mean, std, model_confidences, augmentation_type=None, augmentation_magnitudes=[]):
    folder_name = f'/home/ekagra/Documents/GitHub/MasterArbeit/non_linear_mapping_data/{augmentation_type}'
    if not os.path.exists(folder_name):
        os.makedirs(folder_name)
    filename = os.path.join(
    ...
    plt.plot(augmentation_magnitudes, mean, "-", color="red", label="Comparison Confidence")
    # plt.errorbar(x, mean, yerr=std, fmt="o")
    plt.fill_between(augmentation_magnitudes, [m - s for m, s in zip(mean, std)],
                     [m + s for m, s in zip(mean, std)], color="red", alpha=0.2)
    plt.plot(augmentation_magnitudes, model_confidences, "-", color="blue", label="Model Confidence")
