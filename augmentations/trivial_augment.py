...
class CombinedTrivialAugmentWide(torch.nn.Module):
    def __init__(
            self,
            custom: bool = False,
    ...
    def apply_standard_augmentation(self, im: Tensor) -> Tuple[Tensor, Dict[str, float]]:
        fill = self.fill
    ...
    def __call__(self, im: Optional[Image.Image]) -> Optional[Tuple[Image.Image, List[float]]]:
        if self.custom:
            augment_im, augment_info = self.apply_custom_augmentation(im)
            return augment_im, augment_info
        else:
