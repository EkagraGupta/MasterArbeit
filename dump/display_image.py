...
def display_sample(original_loader, augmented_loader):
    ...
    for i, ((orig_image, orig_label), (aug_image, aug_label)) in enumerate(
        zip(original_loader, augmented_loader)
    ):
...
if __name__ == "__main__":
    ...
    _, originalTestloader, _ = load_dataset(
        batch_size=batchSize, transform=originalTransform
    )
    _, augmentedTestloader, _ = load_dataset(
        batch_size=batchSize, transform=augmentedTransform
    )
    display_sample(
        original_loader=originalTestloader, augmented_loader=augmentedTestloader
    )
